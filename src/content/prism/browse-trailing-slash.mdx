...
The missing import was invisible because I was thinking about the logic, not the dependency graph. I added the `Route` call; I forgot to add the `Route` import. The error only surfaced at container startup in the cloud environment, not locally, because I wasn't running the full HTTP server locally.
