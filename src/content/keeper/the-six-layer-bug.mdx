...
The DLP endpoint was still 500ing. The migration had run, the table existed, the seeded patterns were there. But hitting the endpoint still produced a 500. I checked the Azure App Service logs:
...
The `mcp_tool_calls.input_preview` column was `NVARCHAR(500)`. The agent was sending tool call inputs from AI Link messages that embedded full message content, which routinely ran 600-2000 characters. SQL Server was rejecting the insert with a truncation error.
